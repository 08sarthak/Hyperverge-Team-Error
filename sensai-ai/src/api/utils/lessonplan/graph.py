...
from api.models import (
    LessonPlanStateWithReview, ReviewResult
)
from api.utils.lessonplan.agent import generate_lesson_plan_detailed, generate_lesson_plan_points, generate_quiz_assignment
from api.utils.lessonplan.fetchlink import fetch_resources
...
def graph_struct():
    ...
    builder = StateGraph(LessonPlanStateWithReview)

    async def generate_user_prompt(state : LessonPlanStateWithReview):
    ...
    async def points_lesson_plan_generator(state : LessonPlanStateWithReview):
        #print("GENERATING LESSON PLAN IN PONTS")
        lesson_plan =await generate_lesson_plan_points(state)
        #print("LESSON PLAN in PONTS GENERATED" , lesson_plan)
        temp_generated_content["lesson_plan_points"] = lesson_plan
        blocked_lesson_plan = temp_generated_content["lesson_plan_points"].split("=====")


    async def detailed_lesson_plan_generator(state: LessonPlanStateWithReview):
        ...
        for block in range(1, block_no + 1):
            ...
            lesson_plan = await generate_lesson_plan_detailed(modified_prompt,state["structured_output"])
            #print("LESSON PLAN TYPE",type(lesson_plan))
            #lesson_plan = json.loads(str(lesson_plan))
            #print("AFTER LESSON PLAN",lesson_plan)
            if topics and len(topics) >= block:
                # Convert list to string for fetch_resources
                topic_str = ", ".join(topics[block-1]) if topics[block-1] else "general"
                web_resources = await fetch_resources(state, topic_str)
            else:
                web_resources = await fetch_resources(state, "general")
            
            if state["quiz"] or state["assignment"]:
                quiz_assignment_content = await quiz_assignment_generator(state,block,block_no,lesson_plan)
            else:
                quiz_assignment_content = None
            ...
            if state["structured_output"] == True:
                ...
                if isinstance(lesson_plan, dict):
                    ...
                    if quiz_assignment_content:
                        lesson_plan["Quiz_/_Assignment"] = [quiz_assignment_content]
    ...
    async def review_lesson_plan_node(state: LessonPlanStateWithReview):
    ...
    async def quiz_assignment_generator(state: LessonPlanStateWithReview,block,block_no,lesson_plan):
        ...
        if quiz_assign_modified_prompt == None:
            ...
            content = await generate_quiz_assignment(modified_prompt_quiz_assignment,state["structured_output"])
            return content

    def should_revise_decision(state: LessonPlanStateWithReview):
...
async def run_graph(state:LessonPlanStateWithReview):
    #result=[]
